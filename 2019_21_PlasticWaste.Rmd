...
bar_waste <- df_plastic_prop %>% 
  ...
  ggplot(aes(fct_reorder(Entity, waste_pc), waste_pc)) + 
    ...
    geom_hline(yintercept = 0, color = "grey40", size = 0.1) +
    coord_flip() +
...
title <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  labs(x = NULL, y = NULL,
       ...
       subtitle = "Middle- and low income countries tend to generate high amounts of mismanaged waste which is at high risk of entering the ocean.\n") +
...
caption <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  labs(x = NULL, y = NULL,
       caption = '\n\n\nVisualization by @CedScherer  |  Data: Ritchie & Roser (2019), OurWorldInData.org') +
  theme(line = element_blank(),
        panel.background = element_rect(fill = NA),
        plot.background = element_rect(fill = NA, color = NA),
        panel.border = element_rect(color = NA),
...
title + bar_waste + map_waste_prop + bar_mismanaged + caption +
  plot_layout(widths = c(0, 0.25, 1, 0.25, 0), nrow = 1)

ggsave("./plots/2019_21/2019_21_PlasticWaste.pdf", width = 14, height = 5.3, device = cairo_pdf)
```
