...
p_stellar <- 
  ...
  ggplot(aes(points_sum, scores, color = year)) + 
    ...
    rcartocolor::scale_color_carto_c(palette = "Emrld", direction = -1, guide = "none") +
    coord_capped_cart(bottom = "both", left = "both") +
    theme(axis.text = element_text(color = "grey27", family = "Roboto Mono"),
          axis.title.x = element_text(size = 18, hjust = .98, 
                                      family = "Passion One", 
                                      color = "grey40", face = "plain"),
          axis.title.y = element_text(size = 18, hjust = .985, 
                                      family = "Passion One", 
                                      color = "grey40", face = "plain"),
          axis.ticks = element_line(color = "grey27"),
          axis.line = element_line(color = "grey27"),
          strip.text = element_text(size = 24, vjust = 1, face = "plain", 
                                    family = "Passion One", color = "grey40"),
          panel.border = element_blank(),
          plot.caption = element_text(size = 15, family = "Passion One", 
                                      color = "grey40")) +
    labs(x = "\nPoints scored", y = "Goals scored\n",
...
p_title <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  labs(x = NULL, y = NULL,
       ...
       subtitle = "Number of goals versus points scored per national team and FIFA World Cup.\n",
...
p_stellar_clean <- 
  ...
  ggplot(aes(points_sum, scores, color = year)) + 
    ...
    geom_point(data = df_outcomes_stats %>% 
               ...
               aes(points_sum, scores), size = 15, shape = "★") +
    facet_wrap(~ country, ncol = 5) +
    scale_x_continuous(limits = c(0, 27), expand = c(0.05, 0.05)) +
    scale_y_continuous(limits = c(0, 27), expand = c(0.05, 0.05)) +
    rcartocolor::scale_color_carto_c(palette = "Emrld", direction = -1, guide = "none") +
    #coord_capped_cart(bottom = "both", left = "both") +
    theme(axis.text = element_blank(),
          axis.title.x = element_text(size = 18, hjust = .98, family = "Passion One", color = "grey40", face = "plain"),
          axis.title.y = element_text(size = 18, hjust = .985, family = "Passion One", color = "grey40", face = "plain"),
          axis.ticks = element_blank(),
          strip.text = element_text(size = 24, vjust = 1, face = "plain", family ="Passion One", color = "grey40"),
          panel.border = element_rect(color = "transparent"),
          plot.caption = element_text(size = 15, family = "Passion One", color = "grey40")) +
    labs(x = "\nPoints scored", y = "Goals scored\n",
         caption = "\nVisualization by Cédric Scherer  |  Data: data.world     \n\n\n\n\n")
```
