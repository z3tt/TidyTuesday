---
title: "TidyTuesday 2020/27 - X-Men Locations by Claremont Run Project"
...
## factorized data in logn format
df_best_stream_fct <-
  df_best_stream %>% 
  bind_rows(df_smooth) %>% 
  mutate(
...
texts <-
  tibble(
    issue = c(295, 80, 245, 127, 190),
    value = c(-35, 37, 30, 57, 55),
    ...
    text = c(
      ...
      '**Wolverine is the most popular X-Men** and has a regular presence in the X-Men comics between 1971 and 1991.',
...
g <- 
  ...
  ggplot(aes(
    ...
    scale_x_continuous(
      limits = c(74, NA),
      breaks = c(97, seq(125, 250, by = 25), 280),
...
g +
  geom_textbox(
    ...
    aes(
      ...
      label = text,
      color = char_costume,
      color = after_scale(darken(color, .07, space = "HLS")),
      vjust = vjust
    ),
    family = "Reem Kufi",
    size = 2.7,
