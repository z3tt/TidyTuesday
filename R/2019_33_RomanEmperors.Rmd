...
theme_update(line = element_blank(),
             ...
             plot.subtitle = element_text(color = "grey30", hjust = 0.5, size = 8, margin = margin(0, 0, 0, 0)),
...
legend <- tibble(
  y = c(-50, -45, -40),
  text = c("Natural Mortality", "Fatality", "Unknown Cause")
)
...
df_emperors %>% 
  mutate(
    ...
    cause = case_when(
      cause == "Natural Causes" ~ "Natural Mortality",
      cause == "Unknown" ~ "Unknown Cause",
      TRUE ~ "Fatality"
    ),
    cause = fct_relevel(cause, "Natural Mortality", "Fatality", "Unknown Cause")
  ) %>% 
  ggplot(aes(x = 1.6, y = reign_start)) + 
    geom_segment(data = labs_yrs, aes(x = 1.6, xend = 1.7, y = time, yend = time), size = 0.5, color = "grey30") +
    geom_segment(x = 1.6, xend = 1.6, y = 55, yend = -405, lineend = "round", size = 2.5, color = "grey30") +
    ggrepel::geom_text_repel(aes(label = name, color = cause), segment.color = "grey60",
                             segment.size = 0.1, family = "Cinzel", fontface = "bold",
                             size = 4, xlim  = c(0, 1.1), hjust = 1) +
    ...
    geom_text(data = legend, aes(x = 0.2, y = y, label = text, color = text), 
              family = "Cinzel", hjust = 0, size = 4) + 
    ...
    labs(x = NULL, y = NULL, title = "The Emperors of Rome",
         subtitle = "\nTimeline of roman emperors, their start of their reign and their cause of death.\nThe darker the circles, the more emperors were reigning during this period.",
         caption = "\nVisualization by @CedScherer  |  Data: Wikipedia Zonination")

ggsave("./plots/2019_33/2019_33_RomanEmperors.pdf", width = 7, height = 22, device = cairo_pdf)
```
