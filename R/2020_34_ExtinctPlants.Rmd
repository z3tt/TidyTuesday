...
df_iucn_threat <- 
  ...
  unique() %>% 
  complete(nesting(continent, binomial_name, group, year_last_seen), id = seq(7, 14, by = .25), fill = list(case = 0)) %>% 
  mutate(
...
df_iucn_action <-
  ...
  unique() %>% 
  complete(nesting(continent, binomial_name, group, year_last_seen), id = seq(0, 6, by = .5), fill = list(case = 0)) %>% 
