...
df_vocabulary <- readr::read_csv(here::here("data", "HipHop_VocabChart_full.csv")) %>% 
...
df_songs_rap <- 
  ...
  mutate(
    ...
    era = case_when(
      #year >= 1973 & year < 1988 ~ "Old-school DJ Era",
...
## Vocabulary joined wiht Spotify data by era
df_songs_words <- 
  df_songs %>% 
...
pop_words <-
  ...
  mutate(
    ...
    era_artist = case_when(
      #year_artist >= 1973 & year_artist < 1988 ~ "Old-school DJ Era",
  ...
  mutate(era_artist = factor(era_artist, levels = c("Golden Age", "Bling-Bling Era", "Internet Era"))) %>% 
  ...
  ggplot(aes(year, duration_m)) +
    ...
    theme(axis.line.x = element_blank(),
          ...
          panel.grid.major.x = element_line(color = "grey25", 
                                            linetype = "dotted", 
                                            size = .4),
          legend.position = c(.17, .25),
          legend.title = element_text(face = "bold", color = "grey80"),
          legend.text = element_text(color = "grey55"),
          plot.margin = margin(20, 100, 10, 0))
```
...
title <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  ...
  theme(line = element_blank(),
        ...
        axis.line.x = element_blank(),
        plot.title = element_markdown(family = "Montserrat ExtraBold", size = 36, 
...
caption <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  ...
  theme(line = element_blank(),
        axis.text = element_blank(),
        axis.line.x = element_blank(),
        plot.title= element_markdown(family = "Montserrat ExtraBold", size = 11, 
...
text_words <- ggplot(data.frame(x = 1:2, y = 1:10)) +
  labs(x = NULL, y = NULL,
       title = "Popular rappers from nowadays, however, use way less unique words. Is that the reason for the higher popularity?") +
  theme(line = element_blank(),
        axis.text = element_blank(),
        axis.line.x = element_blank(),
